...
idx_t TableCatalogEntry::GetColumnsSize(bool is_regular_columns) {
	idx_t result = 0;
	if (is_regular_columns) {
		if (rai_column_index == 0) {
			for (auto i = 0u; i < columns.size(); i++) {
				auto regular_column_size = storage->GetColumn(i)->GetSizeInBytes();
				cout << "Regular column " << columns[i].name << ": " << (double)regular_column_size / (1024.0 * 1024.0)
				     << "MB." << endl;
				result += regular_column_size;
			}
		} else {
			for (auto i = 0u; i < rai_column_index; i++) {
				auto regular_column_size = storage->GetColumn(i)->GetSizeInBytes();
				cout << "Regular column " << columns[i].name << ": " << (double)regular_column_size / (1024.0 * 1024.0)
				     << "MB." << endl;
				result += regular_column_size;
		...
		for (auto i = rai_column_index; i < columns.size(); i++) {
			auto index_column_size = storage->GetColumn(i)->GetSizeInBytes();
			cout << "Join index column " << columns[i].name << ": " << (double)index_column_size / (1024.0 * 1024.0)
			     << "MB." << endl;
			result += index_column_size;
		}
	}
	return result;
}
