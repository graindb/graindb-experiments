...
public:
	...
	idx_t GetSizeInBytes() override {
		idx_t result = 0;
		result += Storage::BLOCK_ALLOC_SIZE;
		auto overflow_header = head.get();
		while (overflow_header) {
			result += overflow_header->size;
			overflow_header = overflow_header->next.get();
		}
		return result;
	}
